...
<BODY onLoad=\"window.setTimeout(&quot;location.href='index.html'&quot;,1000)\"bgcolor=\"#ffffff\">\
...
static unsigned portLONG ulPageHits = 0;
	...
	if( pxRxBuffer != NULL )
	{
      ...
      netconn_write(pxNetCon, webHTTP_OK, (u16_t)strlen( webHTTP_OK ), NETCONN_COPY );
			...
			strcat( cDynamicPage, "<p>Version.Build " );
	    sprintf( cPageHits, "%d.%d", System_GetVersionNumber(), System_GetBuildNumber() ); 
      strcat( cDynamicPage, cPageHits );
      strcat( cDynamicPage, "<p>Free Memory " );
	    sprintf( cPageHits, "%d", System_GetFreeMemory() ); 
      strcat( cDynamicPage, cPageHits );
      strcat( cDynamicPage, "</p>" );

			strcat( cDynamicPage, "<p>Tasks Currently Running" );
			strcat( cDynamicPage, "<p><pre>Task          State  Priority  Stck Rem	#<br>-------------------------------------------" );
			/* ... Then the list of tasks and their status... */
			vTaskList( ( signed portCHAR * ) cDynamicPage + strlen( cDynamicPage ) );	
      int i;

      strcat( cDynamicPage, "<p><pre>Inputs<br>--------------<br>" );

      for ( i = 0; i < 8; i++ )
      {
        char b[ 20 ];
        snprintf( b, 20, "%d: %4d<br>", i, AnalogIn_GetValue( i ) );
        strcat( cDynamicPage, b );
      }

      strcat( cDynamicPage, "</pre>" );

...
/**
  ...
  @param output A pointer to the buffer into which the encrypted data will be written.
  @param outlen The maximum number of bytes to write into the output buffer.
  @param input A pointer to the data to be encrypted.
  @param inlen The number of bytes of data to encrypt.
  @param password The password, or key, used to encrypt the data.  Must be 16 bytes long.
...
int Aes_Encrypt(unsigned char* output, int outlen, unsigned char* input, int inlen, unsigned char* password)
{
  ...
  while(inlen)
  {
    ...
    char c = BLOCK_SIZE - j; // pad with the character that corresponds to how many bytes we need to pad
...
/**
  ...
  @param output A pointer to the buffer into which the decrypted data will be written.
  @param outlen The maximum number of bytes of data to write into the output buffer.
  @param input A pointer to the encrypted block to be decrytped.
  @param inlen The number of bytes to be decrypted.
  @param password The password, or key, used to encrypt the data.  Must be 16 bytes long.
  ...
  int written = Aes_Encrypt(cipherbuf, 256, plaintext, 26, secret);
  written = Aes_Decrypt(plainbuf, 256, cipherbuf, written, secret);
...
int Aes_Decrypt(unsigned char* output, int outlen, unsigned char* input, int inlen, unsigned char* password)
{
  ...
  while(inlen)
  {
    ...
    if(inlen < BLOCK_SIZE) // the ciphered text should be in 16 byte chunks
      return -1;

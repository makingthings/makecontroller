...
/* MakingThings: commented this out to accomodate being able to change the MAC address */
...
xSemaphoreHandle xEMACInit( void )
{
  ...
  /* MAKINGTHINGS: ADDITION */
    ...
    AT91C_BASE_PIOB->PIO_PER = 1 << 13;	
    AT91C_BASE_PIOB->PIO_OER = 1 << 13;
...
static void prvSetupMACAddress( void )
{
  ...
  /* MakingThings: Switch variables to load from */
	AT91C_BASE_EMAC->EMAC_SA1L =	( ( unsigned portLONG ) emacETHADDR3 << 24 ) |
									( ( unsigned portLONG ) emacETHADDR2 << 16 ) |
									( ( unsigned portLONG ) emacETHADDR1 << 8  ) |
									emacETHADDR0;

	AT91C_BASE_EMAC->EMAC_SA1H =	( ( unsigned portLONG ) emacETHADDR5 << 8 ) |
									emacETHADDR4;
}
/*-----------------------------------------------------------*/
